...
- **本番(単一オリジン)**: `npm run build:web` 後、server が `apps/web/dist` を `/` 配信(SPAフォールバック)。server は `cd apps/server && npx tsx src/index.ts` で起動(後述の常駐化が未了)。
...
5. **ライブラリ既存場所の使い回しUI未配線** — API(`addPlaceToTrip` with place_id / `GET /api/places`)はあるが、別の旅へ既存場所を選んで足すUIが無い。
...
9. **テスト/CI 未整備**、**spec(feature/test) 薄い**。
